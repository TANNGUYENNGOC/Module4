...
public interface IContractRepository extends JpaRepository<Contract,Integer> {
    @Query(value = "SELECT contract.id, facility.name as nameFacility, employee.name as nameEmployee, customer.name as nameCustomer,  contract.start_date as startDate, contract.end_date as endDate, contract.deposit,(SUM(attach_facility.cost) + facility.cost) AS sumMoney FROM contract left JOIN contract_detail ON contract.id = contract_detail.contract_id left JOIN attach_facility ON attach_facility.id = contract_detail.attach_facility_id left JOIN facility ON facility.id = contract.facility_id left JOIN customer ON customer.id = contract.customer_id JOIN employee ON employee.id = contract.employee_id GROUP BY contract.id"
